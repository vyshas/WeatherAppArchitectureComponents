...
public class SunshineRepository {
    ...
    private boolean isFetchNeeded() {
        ...
        Date date = SunshineDateUtils.getNormalizedUtcDateForToday();
        int countAllFutureWeather = mWeatherDao.countAllFutureWeather(date);

        return countAllFutureWeather < WeatherNetworkDataSource.NUM_DAYS;
    }
    ...
    public LiveData<WeatherEntry> getWeatherbyDate(Date date) {
        //initialize and get from network if not got from network
        initializeData();
        return mWeatherDao.getWeatherByDate(date);
    }

    public LiveData<List<WeatherEntry>> getForecastAfterDate(Date date) {
        initializeData();
        return mWeatherDao.getWeatherForecastAfterDate(date);
    }
